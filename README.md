...
- **평소**: 입을 벌린 상태로 독에서 대기
- **파일을 던지면**: 즉시 휴지통으로 삭제 → 입을 다물었다가 → 다시 벌림
- **독 아이콘 클릭**: 입을 잠깐 다물었다 벌리는 애니메이션 (삭제 없음)
- **독 아이콘 우클릭**: 고양이 종류(검은 고양이 / 흰 고양이) 선택 — 선택은 다음 실행에도 유지
...
`art/` 폴더의 파일을 바꾼 뒤 `./build.sh`를 다시 실행하면 됩니다. 파일명은 `<종류>_<상태>.png` 규칙입니다.

| 파일 | 역할 |
|---|---|
| `art/black_idle.png` | 검은 고양이 — 입 다문 모습 |
| `art/black_open.png` | 검은 고양이 — 입 벌린 모습 (평소 상태) |
| `art/white_idle.png` | 흰 고양이 — 입 다문 모습 |
| `art/white_open.png` | 흰 고양이 — 입 벌린 모습 (평소 상태) |
| `art/AppIcon.icns` | 앱 아이콘 (미실행 시 표시) |

`CatType`에 항목을 추가하고 같은 규칙으로 이미지를 넣으면 고양이 종류를 더 늘릴 수 있습니다.
